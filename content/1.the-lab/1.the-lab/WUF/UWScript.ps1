...
# Set window title and color scheme of powershell
...
function Show-Header {
    ...
    Write-Host "                  UWScript                  " -ForegroundColor red
    Write-Host "============================================" -ForegroundColor red
    Write-Host "" 
...
function Test-WinGetStatus {
    ...
    function Install-WinGetDependencies {
        Show-Header
        Write-Host "Installing required dependencies, please wait . . ." -ForegroundColor red
    ...
    function Install-WinGet {
        Show-Header
        Write-Host "WinGet is not installed. Downloading the latest version from GitHub..." -ForegroundColor red
        ...
        elseif ($updateCheck -match "An applicable upgrade is available") {
            ...
            Try {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Show-Header
                    Write-Host "Failed to update WinGet. Proceeding with app installation..." -ForegroundColor red
                }
            }
            Catch {
                Show-Header
                Write-Host "An error occurred while upgrading WinGet. Proceeding with app installation..." -ForegroundColor red
            }
        }
        else {
            Show-Header
            Write-Host "Could not determine WinGet update status. Proceeding with app installation..." -ForegroundColor red
        }
    }
    Catch {
        Show-Header
        Write-Host "An error occurred while checking for WinGet updates. Proceeding with app installation..." -ForegroundColor red
    }
}
...
function Remove-Apps {
    Show-Header
    Write-Host "Are You Sure You Want to Remove ALL Windows Apps? (Y/N)" -ForegroundColor Black -Backgroundcolor red
    Write-Host "Includes: OneDrive, Teams, Outlook for Windows and more . . ." -ForegroundColor Black -Backgroundcolor red
...
function Set-ServiceStartup {
    ...
    'RemoteAccess', 'RemoteRegistry', 'shpamsvc', 'tzautoupdate', 'uhssvc',
    'UevAgentService'
	)
    ...
    foreach ($service in $disabledServices) {
        try {
            ...
            Show-Header
            Write-Host "Failed to set $service to Disabled: $_" -ForegroundColor red
            Wait-IfNotSpecialize
        }
    }
    ...
    foreach ($service in $manualServices) {
        try {
            ...
            Show-Header
            Write-Host "Failed to set $service to Manual: $_" -ForegroundColor red
            Wait-IfNotSpecialize
        }
    }
